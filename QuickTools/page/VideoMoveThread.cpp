...
VideoMoveThread::VideoMoveThread()
{
...
void VideoMoveThread::GetFileInfos(const QString& strFilePath)
{
...
QString VideoMoveThread::GetCurrentLog(const QString& strLog)
{
    QString strTime = QDateTime::currentDateTime().toString("[yyyy-MM-dd HH:mm:ss.zzz]");
    return QString("%1 %2").arg(strTime).arg(strLog);
}

void VideoMoveThread::startThread(const QString& strSource, const QString& strDest, const QString& strName)
{
    m_strDest = strDest;
